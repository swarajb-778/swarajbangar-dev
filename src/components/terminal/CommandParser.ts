...
const CAT_ABOUT: readonly string[] = [
  ...
  '',
  '  Full-stack engineer who builds financial systems people trust —',
  '  not demos, but money-critical systems that have to be right.',
  '',
  '  Currently at McKinsey building a treasury & credit-risk platform',
  '  with a GenAI layer; previously ThoughtWorks (fintech lending &',
  '  reconciliation, 500K+ txns/month). Python · React · AWS · GenAI,',
  '  with grounding and security from day one.',
  '',
...
const API_OUTPUT: readonly string[] = [
  ...
  `    ${C.teal}"monthly_transactions"${C.reset}: ${C.gold}"500K+"${C.reset},`,
  `    ${C.teal}"reconciliation_accuracy"${C.reset}: ${C.gold}"97%"${C.reset},`,
  `    ${C.teal}"forecast_accuracy"${C.reset}: ${C.gold}"96%"${C.reset},`,
  ...
  switch (cmd) {
    ...
    case 'about':
      return {
        output: [
          '',
          `  ${C.teal}Full-stack engineer building financial systems people trust.${C.reset}`,
          `  ${C.muted}Currently McKinsey · Previously ThoughtWorks${C.reset}`,
          '',
    ...
    case 'experience':
      return {
        output: [
          '',
          `  ${C.gold}McKinsey${C.reset}      ${C.teal}Full Stack Dev${C.reset}       Jan 2026 – Present`,
          `  ${C.gold}ThoughtWorks${C.reset}  ${C.teal}Sr Full Stack Eng${C.reset}    Dec 2023 – Sep 2024`,
          `  ${C.gold}ThoughtWorks${C.reset}  ${C.teal}Full Stack Eng${C.reset}       May 2021 – Dec 2023`,
          '',
